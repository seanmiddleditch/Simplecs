...
namespace SimplecsTests {
    public class ViewTest {
        ...
        public void Require() {
            var world = new World();
            world.Create().Attach(new Comp2 { x = 1 });
            world.Create().Attach(new Comp1 { name = "Bob" }).Attach(new Comp2 { x = 2 });
            world.Create().Attach(new Comp2 { x = 3 });
            world.Create().Attach(new Comp1 { name = "Susan" }).Attach(new Comp2 { x = 4 });

            var view = new View<Comp2>(world).Require<Comp1>();
        ...
        public void Exclude() {
            ...
            var view = new View<Comp2>(world).Exclude<Comp1>();
